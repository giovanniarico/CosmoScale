...
def RandWalk (point,resolution): #1
    ...
    while nstep<resolution:
        ...
        if np.random.random_sample() < np.sqrt(Err1/Err2)*np.exp((Err2/Err1)**2):
...
while nstep<1000:
    ...
    if np.random.random_sample() < np.sqrt(Err2/Err1)*np.exp((Err1/Err2)**2):
